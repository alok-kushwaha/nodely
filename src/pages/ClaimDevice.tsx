...
export default function ClaimDevice() {
  ...
  const checkDevice = async (deviceUuid: string) => {
    ...
    try {
      ...
      const { data, error: invokeError } = await supabase.functions.invoke('claim-device', {
        body: { device_uuid: deviceUuid, action: 'check' },
        headers: {
      ...
      if (invokeError) {
        ...
        setError(data.error === 'Device not found' ? 'Device not found. Please check the UUID.' : data.error);
      } else if (data?.status === 'claimed') {
        if (data.owned_by_you) {

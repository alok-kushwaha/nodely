...
export default function Admin() {
  ...
  const toggleDeviceLock = async (deviceId: string, locked: boolean) => {
    ...
    const { error } = await supabase
      .from('devices')
      .update({ locked, ...(locked ? { relay_state: false } : {}) })
      .eq('id', deviceId);
    ...
    if (error) {
      ...
      setDevices((prev) =>
        prev.map((d) => (d.id === deviceId ? { ...d, locked, ...(locked ? { relay_state: false } : {}) } : d))
      );
      // Publish MQTT LOCK/UNLOCK for instant ESP response
      if (device) {
        supabase.functions.invoke('mqtt-publish', {
          body: {
  ...
  const toggleDeviceRelay = async (deviceId: string, state: boolean) => {
    ...
    if (error) {
      ...
      // Publish MQTT command for instant ESP response
      if (device) {
        supabase.functions.invoke('mqtt-publish', {
          body: {
            device_uuid: device.device_uuid,

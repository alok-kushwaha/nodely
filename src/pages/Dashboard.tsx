...
import { WiFiConfigDialog } from '@/components/devices/WiFiConfigDialog';
...
export default function Dashboard() {
  ...
  const [wifiTarget, setWifiTarget] = useState<{ deviceUuid: string; deviceName: string } | null>(null);
  ...
  const handleToggleRelay = useCallback(async (deviceId: string, state: boolean) => {
    ...
    // Find device info BEFORE any async work (avoid stale closure)
    ...
    // Debounce: batch rapid toggles within 300ms
    const timer = setTimeout(async () => {
      ...
      if (error) {
        ...
        // Publish MQTT command for instant ESP32 response
        const { error: mqttError } = await supabase.functions.invoke('mqtt-publish', {
          body: {
        ...
        if (mqttError) {
          console.warn('MQTT publish failed (device will pick up via poll):', mqttError);
        }
      }
  ...
  const handleWiFiConfig = useCallback((deviceUuid: string, deviceName: string) => {
    setWifiTarget({ deviceUuid, deviceName });
    setWifiDialogOpen(true);
  }, []);

  const handleWiFiSubmit = useCallback(async (ssid: string, password: string) => {
    if (!wifiTarget) return;
    const { error } = await supabase.functions.invoke('mqtt-publish', {
      body: {
        device_uuid: wifiTarget.deviceUuid,
        command: 'WIFI_UPDATE',
        wifi_ssid: ssid,
    ...
    if (error) {
      toast({
        title: 'Failed to send WiFi update',
        description: error.message,
        variant: 'destructive',
      });
    } else {
      toast({
        ...
        description: 'The device will attempt to connect to the new network.',
      });
    }
  }, [wifiTarget, toast]);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <WiFiConfigDialog
        ...
        deviceName={wifiTarget?.deviceName || ''}
